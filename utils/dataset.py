...
class ND_TWIN_Verify():
    def __init__(self, root, pairs_file, transform=None):
        """
        Dataset for verification pairs
        Args:
            root: Root directory containing images
            pairs_file: Path to test_pairs.txt file
    ...
    def __getitem__(self, idx):
        ...
        # Load images
        img1 = Image.open(os.path.join(self.root, 'ND_TWIN_448', img1_path)).convert('RGB')
        img2 = Image.open(os.path.join(self.root, 'ND_TWIN_448', img2_path)).convert('RGB')
...
class ND_TWIN():
    def __init__(self, root, data_len=None, transform=None):
        self.root = root
        self.transform = transform
        
        # Read the required files
        with open(os.path.join(self.root, 'images.txt')) as f:
            img_name_list = [line.strip().split(' ')[-1] for line in f]

        with open(os.path.join(self.root, 'image_class_labels.txt')) as f:
            label_list = [int(line.strip().split(' ')[-1]) - 1 for line in f]  # Convert to 0-based indexing

        with open(os.path.join(self.root, 'train_test_split.txt')) as f:
            train_test_list = [int(line.strip().split(' ')[-1]) for line in f]
        
        # Keep only training images
        train_file_list = [x for i, x in zip(train_test_list, img_name_list) if i]
        train_label_list = [x for i, x in zip(train_test_list, label_list) if i]

        # Apply optional length cutoff
        if data_len is not None:
            train_file_list = train_file_list[:data_len]
            train_label_list = train_label_list[:data_len]

        # Store image paths instead of loading all images
        self.train_img_paths = [os.path.join(self.root, 'ND_TWIN_448', fname) for fname in train_file_list]
        self.train_label = train_label_list
        self.train_imgname = train_file_list
    
    def __getitem__(self, index):
        img_path = self.train_img_paths[index]
        target = self.train_label[index]
        imgname = self.train_imgname[index]

        # Load image lazily with PIL
